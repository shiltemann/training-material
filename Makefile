...
check-html-gh-pages:  ## validate HTML on gh-pages branch (for daily cron job)
	bundle exec htmlproofer --http-status-ignore 405,999 --url_swap "github.com/galaxyproject/training-material/tree/master:github.com/$TRAVIS_REPO_SLUG/tree/$BRANCH" --url-ignore "/.*localhost.*/","/.*vimeo\.com.*/" --file-ignore "/.*\/files\/.*/" .
